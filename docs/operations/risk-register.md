...
| ----- | ---------------------------------------------------------------------------------------------------------------------- | --------- | ---------------------- | ---------------------------------------------------------------------------------------------------------- |
| R-001 | El bootstrap vigente crea `parcelas.subsector_id`, pero la migración histórica 001 todavía intenta indexar `sector_id` | Crítica   | Reabierto              | Preparar spec correctiva del bootstrap; el smoke volvió a fallar antes de migraciones nuevas el 2026-09-07 |
...
| R-023 | El catálogo productivo puede no tener exactamente un Mango activo, usar un código distinto al canónico `MNG` o no tener un nivel único por tipo y grado 0..3 | Alta | Mitigado | Las migraciones de catálogo validan cardinalidades dentro de la transacción y abortan sin carga parcial; el preflight debe comprobar `cultivos.codigo = 'MNG'`, niveles y backup antes del deploy |
