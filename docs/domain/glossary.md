---
title: Glosario del dominio
status: active
owner: mantenimiento
last_reviewed: 2026-06-25
---

# Glosario

- **Agrónomo:** usuario técnico que registra y atiende visitas.
- **Campaña:** periodo agrícola asociado a un cultivo.
- **Catálogo:** conjunto administrable de valores usados por visitas y recetas.
- **Etapa fenológica:** fase del desarrollo del cultivo.
- **Incidencia:** presencia o extensión de una plaga, enfermedad o deficiencia.
- **Labor cultural:** actividad de manejo realizada o recomendada en la parcela.
- **Outbox:** cola local de operaciones pendientes de sincronización.
- **Parcela:** unidad agrícola perteneciente a un productor y sector.
- **Productor:** persona responsable o propietaria de parcelas.
- **Receta:** recomendación consolidada de fitosanidad, fertilización, riego y
  labores.
- **Sector:** unidad territorial dentro de un distrito.
- **Severidad:** intensidad del daño observado.
- **Subetapa:** detalle dentro de una etapa fenológica.
- **Visita de campo:** registro agronómico central realizado sobre una parcela.
