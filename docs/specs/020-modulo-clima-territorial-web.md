...
- Solo ADMIN puede ver y operar las siete vistas y las rutas `/clima/*`.
...
- Acceso o administración por roles distintos de ADMIN.
...
- RF-001: El padre Clima es un grupo expandible sin redirección; las siete rutas
  hijas aparecen en el orden aprobado y se protegen en frontend y API.
...
- RF-003: Resumen presenta condición territorial, mínimo/máximo, humedad,
  ...
  alertas activas y estado de fuentes.
- RF-004: Mapa usa Leaflet ya instalado y solo visualiza puntos climáticos y
  estaciones; admite filtros por variable, fuente, tipo de dato y periodo.
- RF-005: Pronóstico presenta 72 h horario y 7–15 días diarios cuando la fuente
  lo permita. Historial ofrece diario, semanal, mensual, anual y CSV.
...
- PostgreSQL: migración expansiva con esquema `clima`, entidades TypeORM e
  ...
  instante.
- API OpenAPI: endpoints ADMIN `/clima/resumen`, `/clima/mapa`,
...
- Cada controlador climático usa `@Roles("ADMIN")`; el control de UI no sustituye
  esa restricción.
...
- [ ] CA-001: El menú padre Clima no navega y sus siete hijos ADMIN aparecen en
  el orden aprobado.
...
- [ ] CA-003: Resumen, mapa, pronóstico e historial distinguen tipo, fuente y
  fecha de actualización; un fallo muestra último dato válido o estado claro.
...
- [ ] CA-007: Un AGRONOMO recibe 403 de la API climática y acceso restringido web.
...
- UI/E2E para navegación, rutas ADMIN, filtros, mapa, estados vacíos y vencidos.
