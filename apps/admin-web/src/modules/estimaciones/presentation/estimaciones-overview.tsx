...
export function EstimacionesOverview() {
  ...
  return (
    <section className="panel-grid estimations-screen">
      <article className="panel estimations-hero">
        <ToolbarActions
          description="Define la meta de cada ingeniero y compárala con las visitas registradas en la misma semana."
        ...
        <div className="estimations-weekbar">
        ...
        {data ? (
          <div aria-label="Totales de la semana" className="estimations-summary">
            <SummaryMetric
      ...
      {!error && !isLoading && data && data.rows.length > 0 ? (
        <article className="panel estimations-table-panel">
          <header className="estimations-table-panel__header">
            <div>
              <h3>Planilla de la semana</h3>
              <p>
                Deja una celda vacía para retirar la estimación. El valor cero es una meta
                explícita y su cumplimiento se muestra como no aplicable.
              </p>
            </div>
          ...
          <div className="data-table__wrapper estimations-table-wrapper">
            <table className="data-table estimations-table">
              <caption>Estimaciones y cumplimiento por ingeniero</caption>
              <thead>
                <tr>
                  <th>Ingeniero</th>
                  <th>Estimadas</th>
              ...
              <tbody>
                {data.rows.map((row) => (
                  <tr
                    className={row.isEditable ? undefined : "estimations-table__inactive"}
...
function SummaryMetric({
  ...
  return (
    <div className={`estimations-summary__item estimations-summary__item--${tone}`}>
      {icon}
      <span>{label}</span>
      <strong>{value}</strong>
    </div>
