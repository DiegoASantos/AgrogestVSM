...
export class ClimaService {
  ...
  async map() {
    await this.syncCurrentIfNeeded();
    const points = await this.points();
    return createSuccessResponse(await Promise.all(points.map(async (point) => ({ ...toPoint(point), current: await this.latest(point.id) }))));
  }
  ...
  async syncOpenMeteo() {
    ...
    try {
      ...
      await this.dataSource.query(`UPDATE clima.fuentes_datos SET estado='DEGRADADA',ultimo_error=$1,actualizado_at=now() WHERE codigo='open_meteo'`, [error instanceof Error ? error.message.slice(0, 500) : 'Fallo de sincronización']);
    } finally { this.syncing = false; }
  }

  private async fetchPoint(sourceId: string, point: Punto) {
    const url = new URL('https://api.open-meteo.com/v1/forecast');
    url.search = new URLSearchParams({ latitude: point.latitud, longitude: point.longitud, timezone: 'America/Lima', forecast_days: '7', current: 'temperature_2m,relative_humidity_2m,precipitation,wind_speed_10m', daily: 'temperature_2m_max,temperature_2m_min,precipitation_sum,precipitation_probability_max,et0_fao_evapotranspiration,wind_speed_10m_max' }).toString();
    ...
    for (const [variable, unit] of [['temperature_2m','°C'],['relative_humidity_2m','%'],['precipitation','mm'],['wind_speed_10m','km/h']] as const) {
      const value = numeric(current[variable]); if (value !== null) await this.insertReading(sourceId, point.id, variable, value, unit, 'ESTIMADO', time);
    }
    const dates = Array.isArray(daily.time) ? daily.time : [];
    const fields: Array<[string,string]> = [['temperature_2m_max','°C'],['temperature_2m_min','°C'],['precipitation_sum','mm'],['precipitation_probability_max','%'],['et0_fao_evapotranspiration','mm'],['wind_speed_10m_max','km/h']];
    for (let index=0; index<dates.length; index+=1) for (const [variable,unit] of fields) { const values = daily[variable]; const value = Array.isArray(values) ? numeric(values[index]) : null; if (value !== null) { const validAt = `${dates[index]}T12:00:00-05:00`; await this.dataSource.query(`INSERT INTO clima.pronosticos(fuente_id,punto_climatico_id,variable,valor,unidad,valido_at,emitido_at,modelo) VALUES($1,$2,$3,$4,$5,$6,now(),'best_match') ON CONFLICT(fuente_id,punto_climatico_id,variable,valido_at,emitido_at) DO NOTHING`, [sourceId,point.id,variable,value,unit,validAt]); await this.createAlerts(sourceId, point.id, variable, value, unit, validAt); } }
  }
  ...
  private async createAlerts(sourceId: string, pointId: string, variable: string, value: number, unit: string, startsAt: string) {
    const rules = await this.dataSource.query(`SELECT id,codigo,operador,valor_precaucion AS precaution,valor_alta AS high,valor_critica AS critical FROM clima.umbrales_alerta WHERE activo=true AND variable=$1`, [variable]);
  ...
  private async point(publicId: string) { const rows = await this.dataSource.query(`SELECT id,public_id,nombre,departamento,distrito,latitud,longitud FROM clima.puntos_climaticos WHERE public_id=$1 AND activo=true`, [publicId]); if (!rows[0]) throw new NotFoundException('Punto climático no encontrado.'); return rows[0] as Punto; }
}
function numeric(value: unknown) { return typeof value === 'number' && Number.isFinite(value) ? value : null; }
function resolveSeverity(value: number, rule: { operador: string; precaution: string | null; high: string | null; critical: string | null }) { const matches = (threshold: string | null) => threshold !== null && (rule.operador === '>=' ? value >= Number(threshold) : value <= Number(threshold)); if (matches(rule.critical)) return 'CRÍTICA'; if (matches(rule.high)) return 'ALTA'; if (matches(rule.precaution)) return 'PRECAUCIÓN'; return null; }
