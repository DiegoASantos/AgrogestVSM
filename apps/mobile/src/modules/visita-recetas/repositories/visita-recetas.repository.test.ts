...
const recetaRow = {
  local_id: "r1", server_id: "srv-1", visita_local_id: "v1", etapa_fenologica: null,
  version: 1, sync_status: "synced" as const, sync_error_message: null,
...
describe("visitaRecetasRepository", () => {
  ...
  describe("#getCoadyuvantes", () => {
    it("should query coadyuvantes ordered by name", () => {
      database.getAllSync.mockReturnValue([{ id: "co1", name: "Aceite agricola", description: null, is_active: 1 }] as never);
  ...
  describe("#getIngredientesActivos", () => {
    it("should query ingredientes activos", () => {
      database.getAllSync.mockReturnValue([{ id: "ia1", name: "Glifosato", is_active: 1 }] as never);
  ...
  describe("#getFertilizantes", () => {
    it("should query fertilizantes with defaults fallback", () => {
      database.getAllSync.mockReturnValue([{ id: "f1", name: "Urea", type: "solido", concentracion: null, unidad_medida: null, is_active: 1 }] as never);
  ...
  describe("#getMarcasProducto", () => {
    it("should query marcas producto", () => {
      database.getAllSync.mockReturnValue([{ id: "mp1", name: "Marca A", is_active: 1 }] as never);
  ...
  describe("#getTiposControl", () => {
    it("should query tipos control ordered by name", () => {
      database.getAllSync.mockReturnValue([{ id: "tc1", name: "Quimico", is_active: 1 }] as never);
