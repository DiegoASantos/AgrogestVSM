...
export function useSync() {
  const { effectiveMode, isOnline, isPhysicallyOnline } = useConnectivity();
  ...
  const executeSync = useCallback(
    async (options: SyncRequestOptions = {}): Promise<SyncRunResult> => {
      ...
      if (!isOnlineRef.current || !isAuthenticatedRef.current) {
        const offlineMessage = !isPhysicallyOnline
          ? "Sin conexion. Tus datos siguen guardados localmente."
          : effectiveMode === "offline_manual"
            ? "Modo offline seleccionado. Tus datos siguen guardados localmente."
            : "Red inestable. AgroGest esta trabajando con datos locales.";
      ...
      try {
        ...
        if (onlineSession !== "valid") {
          ...
          return finishRun(createSyncRunResult(status, message));
        }
    ...
    [effectiveMode, finishRun, isPhysicallyOnline]
  );
  ...
  const scheduleSync = useCallback(
    (options: SyncRequestOptions = {}) => {
      ...
      const adaptiveDelayMs = options.bypassBackoff
        ? 0
      ...
      return new Promise<SyncRunResult | null>((resolve) => {
        ...
        scheduledSyncRef.current = setTimeout(async () => {
